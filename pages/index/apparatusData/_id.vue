    ...
    <div>
      ...
      <div style="margin-bottom: 20px">
        <a-descriptions bordered :column="{ xxl: 4, xl: 3, lg: 3, md: 3, sm: 2, xs: 1 }">
          <a-descriptions-item label="名字">{{ apparatusEntityData.name }}</a-descriptions-item>
          <a-descriptions-item label="采购单号">{{ apparatusEntityData.billNo }}</a-descriptions-item>
          <a-descriptions-item label="序列号">{{ apparatusEntityData.seqId }}</a-descriptions-item>
          <a-descriptions-item label="类">{{ apparatusEntityData.familyName }}</a-descriptions-item>
          <a-descriptions-item label="模组">{{ apparatusEntityData.model }}</a-descriptions-item>
          <a-descriptions-item label="料号">{{ apparatusEntityData.partNo }}</a-descriptions-item>
          <a-descriptions-item label="库位">{{ apparatusEntityData.location }}</a-descriptions-item>
          <a-descriptions-item label="状态">{{ apparatusEntityData.status }}</a-descriptions-item>
          <a-descriptions-item label="当前位置">信号丢失或未安装定位装置</a-descriptions-item>
        </a-descriptions>
      </div>
    </div>
    <div>
      ...
      <a-timeline style="margin: 30px 30px 0">
        <a-timeline-item :color="getColor(timelineItem.type)" v-for="timelineItem in timeline">
          <a-icon
            ...
            v-if="hasClockCircle(timelineItem.type)"
          />
...
export default {
  data() {
    ...
    return {
      timeline: [],
      histogramChartData: {
        columns: ["寿命", "预计最大剩余寿命", "预计最小剩余寿命", "最可能剩余寿命"],
        rows: [
          {
            寿命: "剩余寿命(小时)",
            预计最大寿命: 900,
      ...
      ringChartData: {
        columns: ["工作时间", "时间(小时)"],
        rows: [
          { 工作时间: "正常工作时间(小时)", "时间(小时)": 3530 },
          { 工作时间: "异常时间(小时)", "时间(小时)": 90 }
        ]
      },
      apparatusEntityData: {
        billNo: "",
    ...
    processTimeline(data) {
      ...
      this.timeline = [
        ...
        if (aTime < bTime) {
          ...
          const bType = b.type;
          const priorityValue =
            timePriority.indexOf(aType) - timePriority.indexOf(bType);
          if (priorityValue > 0) {
            return -1;
        ...
        changeKeys.forEach(key => {
          ...
          if (changeValue.changed !== "equal" && changeValue.added) {
            ...
            if (key === "Picture") {
              ...
              typeof changeValue.added === "string" &&
              changeValue.added.slice(0, 4) === "0001"
            ) {
              // console.log('hello')
            } else {
